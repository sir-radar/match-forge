...
Sprint 2 baseline, walk-forward, calibration, artifact, forecast, evaluation, and governance contracts are implemented. `make sprint2-evaluate` resolves one validated lineage, freezes the target plan, fits and persists each chronological batch before outcome reveal, and retains JSON, Parquet, and SVG evidence. The approved EPL 2015/16 corpus contains 380 matches and 1,313,773 normalized events. Contradictory player facts retain exact variants; validator v3 records 749 warnings and no quarantine findings. Separate versioned claims bind completion, UTC chronology, and corner labels to exact provider, dataset, validation, competition, local-time, event, and pinned timezone evidence without mutating raw provider facts. An immutable label-free target plan applies the frozen 10-match team and 100-match competition history rules using prior batches only, yielding 280 targets across 146 eligible batches after 100 warm-up exclusions. Separate outcome reveal covers 280/280 frozen targets. Successful evidence production intentionally stops at `FAIL / baseline-policy-review`; no policy threshold is changed and no model or calibration challenger is promoted automatically. Player modelling and full Monte Carlo simulation remain blocked until Sprint 2 evaluation evidence is reviewed and passed.
